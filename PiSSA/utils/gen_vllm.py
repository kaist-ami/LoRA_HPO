...
elif args.model.count("/") >= 2:
    raise SystemExit(
        f"Model directory not found: {args.model} (cwd={os.getcwd()}). "
        "The training step likely failed before saving the merged model."
    )
...
if args.sub_task is None:
    ...
    for task in args.sub_task:
        split_file = os.path.join(args.data_path, task, args.dataset_split + ".json")
        ds = load_dataset("json", data_files=split_file, split="train")
