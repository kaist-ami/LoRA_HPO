...
class TrainingArguments(transformers.TrainingArguments):
    ...
    use_rslora : Optional[bool] = field(default=False, metadata={"help": "Use rank-stabilized LoRA (rsLoRA)."})
...
def build_model(script_args, checkpoint_dir):
    ...
    if getattr(script_args, "gradient_checkpointing", False):
...
def train():
    ...
    for task in script_args.sub_task:
        ...
        base_split = cur_split.split("[")[0]
        split_file = os.path.join(script_args.data_path, cur_task, base_split + ".json")
        if not os.path.exists(split_file):
            raise FileNotFoundError(
                f"{split_file} not found. Only test.json is shipped; add "
                f"{base_split}.json for task '{cur_task}' (see README section 'Data')."
            )
        slice_suffix = cur_split[len(base_split):]
        ds = load_dataset("json", data_files=split_file, split="train" + slice_suffix)
        if script_args.local_rank == 0:
